param(
    ...
    [string]$MihomoDir = (Join-Path $PSScriptRoot "mihomo"),
    [switch]$Clean
)
...
# Ensure mihomo source repository is available
if (-not (Test-Path -LiteralPath $mihomoRoot -PathType Container)) {
    Write-Host "mihomo source not found at: $mihomoRoot"
    Write-Host "Cloning metacubex/mihomo (Alpha branch)..."
    & git clone --depth 1 https://github.com/metacubex/mihomo -b Meta $mihomoRoot
...
if ($null -eq $goCmd) {
    throw "Go toolchain not found. Please install Go from https://go.dev/dl/ and ensure 'go' is in PATH."
}
Write-Host "Using Go: $((& go version).Trim())"
